...
def build_single_post_email(title, description, image_url, link):
    description_html = (
        f'<p style="margin-top:12px;font-size:16px;color:{LIGHT_TEXT_MUTED};">{description}</p>'
        if description
        else ""
    )

    image_html = (
        f"""    <div style="margin:40px 0 0 0;">
      <img src="{image_url}"
        alt="{title}"
        style="
          width:100%;
    ...
    return SINGLE_POST_TEMPLATE.format(
        title=title, description=description_html, image=image_html, link=link
    )
...
def get_week_start():
    ...
    week_start = week_start - timedelta(
        days=days_since_sunday if days_since_sunday < 7 else 7
    )
    return week_start
